/**
 * azure_devops_create_test_run — Create a new test run from plan + suite point IDs.
 */

import { Type } from "typebox";
import { resolveConfig, type AzureDevOpsConfig } from "../config/index.ts";
import { getTestPlanApi, getTestResultsApi } from "../utils/connection.ts";
import { formatAdoError } from "../utils/errors.ts";
import { formatTestRun } from "../utils/formatting.ts";
import { isMock, textResult, errorResult, type ToolResult , resolveEffectiveConfig, OrgParam, ProjectParam} from "./shared.ts";
import { mockCreateTestRun } from "../mocks/mock-handler.ts";

export const createTestRunTool = {
	name: "azure_devops_create_test_run",
	description:
	...
	promptGuidelines: [
		"Use azure_devops_create_test_run to start a manual test execution session.",
		"You need the plan ID and at least one suite ID. Use azure_devops_list_test_suites first to find suite IDs.",
	],
